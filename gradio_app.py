...
class canny2image:
    def __init__(self, device):
        ...
        model = create_model("ControlNet/models/cldm_v15.yaml", device=device).to(
            device
        )
...
class scribble2image:
    def __init__(self, device):
        ...
        model.load_state_dict(
            load_state_dict(
                "ControlNet/models/control_sd15_scribble.pth", location="cpu"
            )
        )
        self.model = model.to(device)
        self.device = device
...
class pose2image:
    def __init__(self, device):
        ...
        model.load_state_dict(
            load_state_dict(
                "ControlNet/models/control_sd15_openpose.pth", location="cpu"
            )
        )
        self.model = model.to(device)
        self.device = device
...
from visual_chatgpt.tools.image_generation import ImageGenerationBC
from visual_chatgpt.tools.image_question_answer import (
    ImageCaptionBC,
    ImageQuestionAnswerBC,
)
...
class ConversationBot:
    def __init__(self):
        ...
        self.i2t = ImageCaptionBC(device="cuda:0")
        self.t2i = ImageGenerationBC(device="cuda:0")
        ...
        self.BLIPVQA = ImageQuestionAnswerBC(device="cuda:0")
